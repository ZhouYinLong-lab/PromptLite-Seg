...
\begin{abstract}
Promptable segmentation systems are often evaluated with clean points and boxes, although real prompts are uncertain and the two channels do not admit directly comparable perturbation scales. We present a pre-specified evaluation on all 1,449 PASCAL VOC 2012 validation images. A disjoint, class-balanced 100-image subset of VOC train is used only to calibrate point displacement and box perturbation to matched observable quality targets. We compare Center Color, GrabCut, a robust-superpixel method with three component ablations, a secondary adaptive-superpixel variant, and SAM ViT-B under three prompt modalities. Three primary hypotheses are tested with paired bootstrap (20,000 replicates), sign-flip permutation tests (50,000 permutations), and Holm correction. Robust Superpixel improves over Center Color by 0.0640 IoU (95\% CI [0.0585, 0.0696]); SAM score selection improves over a single noisy prompt by 0.0193 [0.0135, 0.0251]; and matched-quality point-noise IoU exceeds box-noise IoU by 0.1069 [0.0997, 0.1138]. In separately labelled post-confirmatory analyses, the adaptive variant yields +0.0074 [0.0053, 0.0096], while a five-target, 289,800-inference sensitivity curve finds negligible channel difference at quality 0.9 but increasing box-noise disadvantage from 0.0238 to 0.2324 IoU at targets 0.8--0.5. Per-class analysis reveals systematic failure modes: bicycle IoU remains below 0.21 for all CPU methods and below 0.36 even for SAM box-only; all eight GrabCut hard failures occur when the tight box spans the full image and leaves no definite-background pixels. Multi-box consensus gives zero mean gain, while SAM point-only trails CPU methods on several rigid classes. The result is a reproducible, stratified account of prompt-channel failure and component-level evidence strength rather than a state-of-the-art claim.
\end{abstract}
...
\begin{table}[ht]
\centering
\caption{Post-confirmatory adaptive vs.\ fixed SLIC segment count on VOC validation.}
\begin{tabular}{lrrrc}
\toprule
...
\noindent A preliminary 20-sample ADE20K convenience check has a ranking and ablation pattern consistent with VOC; Appendix~\ref{app:ade20k} reports the full table and scope limitations.
...
\section{Preliminary cross-dataset validation on ADE20K}
\label{app:ade20k}

We run the same six CPU methods on the first 20 stream-valid samples from the ADE20K validation mirror \cite{ade20k}. This is a convenience subset, not a representative sample. Samples come from the \texttt{1aurent/ADE20K} Hugging Face mirror with a minimum instance area of 256~px. The loader records a seed for reproducibility, but the current streaming path does not shuffle sample order. Targets and clean prompts follow the VOC protocol.
...
The ranking is GrabCut $>$ Robust Superpixel $>$ Center Color, and the color seed remains the largest contributor. The single-box ablation differs from the multi-box default by only 0.0005~IoU, consistent with the VOC finding that box consensus has essentially no average benefit. This evidence remains preliminary because the subset is small, order-dependent, and not representative, and image resolutions differ materially.

\noindent\textbf{Scale-up status.} A runner that can scan the complete 2,000-row validation stream is implemented in \texttt{scripts/run\_ade20k\_cpu.py}; the 256-px target filter may yield fewer eligible samples. Only 20 are reported because the available environment could not reliably stream the full mirror.
